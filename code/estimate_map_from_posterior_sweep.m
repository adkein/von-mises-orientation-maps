function Qhat_vec = estimate_map_from_posterior_sweep(Qhat_vec,Y_vec,R,kq,ky)
            ...
            for k2=0:k_max(2)
                ...
                for k3=0:k_max(3)
                    ...
                    for k4=0:k_max(4)
                        ...
                        if i > 1
                            h = Q{i-1,j};
                            factor = factor * exp(kq * mu_k1' * h);
                            mu_sum = mu_sum + kq * mu_k1;
                        end
                        if j < N
                            h = Q{i,j+1};
                            factor = factor * exp(kq * mu_k2' * h);
                            mu_sum = mu_sum + kq * mu_k2;
                        end
                        if i < M
                            h = Q{i+1,j};
                            factor = factor * exp(kq * mu_k3' * h);
                            mu_sum = mu_sum + kq * mu_k3;
                        end
                        if j > 1
                            h = Q{i,j-1};
                            factor = factor * exp(kq * mu_k4' * h);
                            mu_sum = mu_sum + kq * mu_k4;
                        end
                        mu_sum = mu_sum + ky * y;
%                         h = Q{i,j};
%                         factor = factor * exp(ky * y' * h);
        ...
        for m = 1:length(prob_vec)
            q_vec = q_vec + prob_vec(m) * mu_vec(:,m) + kq / ky * y;
        end
        q_vec = q_vec;
        cos_sin = q_vec / sqrt(q_vec' * q_vec);
        Q{i,j} = cos_sin;
    end
end
Qhat_vec = Q;
